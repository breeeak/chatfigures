...
from apps.fadmin.serializers import SaveFileSerializer
...
def export_excel_save_model(request, field_data, data, FilName):
    ...
    # 存入文件数据库中，会先查有没有重名文件，有的话就做修改，以防止重名错误，
    ...
    savefile, created = SaveFile.objects.get_or_create(file=file_rul)
    if created:
