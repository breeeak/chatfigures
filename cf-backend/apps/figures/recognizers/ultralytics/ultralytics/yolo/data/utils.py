...
from ultralytics.yolo.utils.ops import segments2boxes, LabelTexts
...
def verify_image_label(args):
    ...
    im_file, lb_file, prefix, keypoint, use_ocrs, num_cls, nkpt, ndim, hyp = args
    # Number (missing, found, empty, corrupt), message, segments, keypoints
    nm, nf, ne, nc, msg, segments, keypoints, ocrs = 0, 0, 0, 0, '', [], None, None
    label_converter = LabelTexts(alphabet=hyp.get('alphabet', None), max_length=hyp.get('seq_length', 30), ignore_case=hyp.get('ignore_case', True))
    try:
        # Verify images
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                if any(len(x) == 6 for x in lb) and use_ocrs:  # is ocr
                    ocrs = [str(x[5]) if len(x) == 6 else "" for x in lb]  # (text...)  "|" for empty
                    ocrs = np.array(label_converter.encode(ocrs), dtype=np.float32)
                    lb = np.array([x[:5] for x in lb], dtype=np.float32)  # (cls, xywh)
            ...
            if nl:
                ...
                for i in range(len(lb)):
                    if lb[i][0] in range(num_cls):
                        valid_indexes.append(i)
                nl = len(valid_indexes)
                if nl:
                    lb = lb[valid_indexes]
                    if segments:
                        segments = [segments[x] for x in valid_indexes]
                    if use_ocrs and ocrs is not None:
                        ocrs = [ocrs[x] for x in valid_indexes]
        ...
        if use_ocrs:
            if (ocrs is None or len(ocrs) == 0):
                ocrs = ["" for _ in range(len(lb))]
                ocrs = np.array(label_converter.encode(ocrs), dtype=np.float32)
            else:
                ocrs = np.array(ocrs, dtype=np.float32)
        lb = lb[:, :5]
        return im_file, lb, shape, segments, keypoints, ocrs, nm, nf, ne, nc, msg
