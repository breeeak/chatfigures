...
def ocr_iou(gt, pred):
    ...
    编辑距离越小，两个串的相似度越大
    ocr_iou 越大，两个串的相似度越大
    """
    result_iou = torch.zeros((len(gt), len(pred)))
    for i, gt_word in enumerate(gt):
        for j, pred_word in enumerate(pred):
            if len(gt_word) == 0 or len(pred_word) == 0:
                result_iou[i, j] = 0.       # 两个串有一个为空串，相似度为0
            else:
                result_iou[i, j] = 1 - edit_distance(gt_word, pred_word) / max(len(gt_word), len(pred_word))
    return result_iou
...
class OcrMetrics(SimpleClass):
    ...
    def __init__(self, save_dir=Path('.'), plot=False, names=()) -> None:
        ...
        self.names = names
        self.box = Metric()
        self.ocr = np.zeros(10)
        self.ocr_acc = 0.0
        self.ocr_acc50 = 0.0
        self.speed = {'preprocess': 0.0, 'inference': 0.0, 'loss': 0.0, 'postprocess': 0.0}

    def process(self, tp_b, ocr_acc, conf, pred_cls, target_cls):
        """
        Processes the detection and ocr metrics over the given set of predictions.

        Args:
            tp_b (list): List of True Positive boxes.
            ocr_acc (Tensor): List of ocr accuracy over different iou.
            conf (list): List of confidence scores.
            pred_cls (list): List of predicted classes.
            target_cls (list): List of target classes.
        """

        self.ocr = np.mean(ocr_acc, axis=0)
        self.ocr_acc = np.mean(ocr_acc)
        self.ocr_acc50 = self.ocr[0]
    ...
    def keys(self):
        ...
        return [
            ...
            'metrics/acc50(O)', 'metrics/acc50-95(O)']

    def mean_results(self):
        """Calculate mean of detected objects & return precision, recall, mAP50, and mAP50-95."""
        return self.box.mean_results() + [self.ocr_acc50, self.ocr_acc]
