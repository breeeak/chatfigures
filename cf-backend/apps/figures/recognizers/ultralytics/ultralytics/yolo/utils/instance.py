...
class Instances:
    ...
    def __init__(self, bboxes, segments=None, keypoints=None, ocrs=None, bbox_format='xywh', normalized=True) -> None:
        """
        Args:
    ...
    def __getitem__(self, index) -> 'Instances':
        ...
        ocrs = self.ocrs[index] if self.ocrs is not None else None
    ...
    def update(self, bboxes, segments=None, keypoints=None, ocrs=None):
    ...
    def concatenate(cls, instances_list: List['Instances'], axis=0) -> 'Instances':
        ...
        use_ocr = instances_list[0].ocrs is not None
        bbox_format = instances_list[0]._bboxes.format
        normalized = instances_list[0].normalized

        cat_boxes = np.concatenate([ins.bboxes for ins in instances_list], axis=axis)
        cat_segments = np.concatenate([b.segments for b in instances_list], axis=axis)
        cat_ocrs = np.concatenate([b.ocrs for b in instances_list], axis=axis) if use_ocr else None
        cat_keypoints = np.concatenate([b.keypoints for b in instances_list], axis=axis) if use_keypoint else None
        return cls(cat_boxes, cat_segments, cat_keypoints, cat_ocrs, bbox_format, normalized)
