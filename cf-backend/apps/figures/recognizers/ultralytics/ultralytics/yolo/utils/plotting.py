        ...
        if len(cls) > 0:
            ...
            if len(bboxes):
                ...
                for j, box in enumerate(boxes.T.tolist()):
                    ...
                    if ocrs is not None and len(ocrs) > i and len(ocrs[i]) > j:
                        c = c + '' + ocrs[i][j]
...
@plt_settings()
def plot_results(file='path/to/results.csv', dir='', segment=False, pose=False, classify=False, ocr=False):
    ...
    if classify:
        ...
        fig, ax = plt.subplots(2, 8, figsize=(18, 6), tight_layout=True)
        index = [1, 2, 3, 4, 5, 6, 7, 8, 11, 12, 13, 14, 9, 10]
    else:
