...
class LabelTexts:   #
    def __init__(self, alphabet: str = None, max_length=None, blank=0, ignore_case=True) -> None:
        if alphabet is None:
        ...
        for i, char in enumerate(iter(self.alphabet)):
            ...
            self.dict[char] = i + 1
        self.dict['<other>'] = len(self.dict) + 1
    ...
    def encode(self, text_list):
        if isinstance(text_list, str):
            ...
            if self.dict['<other>'] in text:
                LOGGER.warning(f'Found <other> in {text_list}')
            if len(text) > self.max_length:
                text = text[:self.max_length]
            else:
                text = text + [0] * (self.max_length - len(text))
            return text
        elif isinstance(text_list, collections.Iterable):
            return [self.encode(text) for text in text_list]

    def decode(self, text_list):
        if len(text_list) == 0:
            return ''
        # if two dim, then decode each row
        if isinstance(text_list, collections.Iterable) and isinstance(text_list[0], collections.Iterable):
            return [self.decode(text) for text in text_list]
        else:
            text = []
            for i in text_list:
                if i == self.blank:
                    ...
                    text.append(self.alphabet[i - 1])
            return ''.join(text)

    def remove_blank(self, labels):
        new_labels = []
        # 合并相同的标签
        previous = None
        for l in labels:
    ...
    def greedy_decode(self, y, l=None, raw=False):
        if l is not None and l<=len(y):
            y = y[:l]
        # 按列取最大值，即每个时刻t上最大值对应的下标
        raw_rs = np.argmax(y, axis=1)
        # 移除blank,值为0的位置表示这个位置是blank
        rs = self.remove_blank(raw_rs)
        if raw:
            return rs
        else:
            return self.decode(rs)
