...
class Results(SimpleClass):
    ...
    def __init__(self, orig_img, path, names, boxes=None, masks=None, probs=None, keypoints=None, ocrs=None) -> None:
        ...
        self.names = names
        self.path = path
        self.ocrs = ocrs if ocrs is not None else None
        self._keys = ('boxes', 'masks', 'probs', 'keypoints', 'ocrs')
        ...
        if pred_boxes and show_boxes:
            for i, d in enumerate(reversed(pred_boxes)):
                ...
                if self.ocrs is not None:
                    ocr = self.ocrs[len(self.ocrs)-i-1]
                    label += " "+ocr
    ...
    def save_txt(self, txt_file, save_conf=False):
        ...
        if probs is not None:
            ...
            for j, d in enumerate(boxes):
                ...
                out_line = ('%g ' * len(line)).rstrip() % line
                if ocrs is not None:
                    out_line += " "+ocrs[j]
                texts.append(out_line)
