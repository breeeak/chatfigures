...
from .block import DFL, Proto, OcrProto
from .conv import Conv
from .transformer import MLP, DeformableTransformerDecoder, DeformableTransformerDecoderLayer
from .utils import bias_init_with_prob, linear_init_
from .crnn import CRNN

__all__ = ['Detect', 'Segment', 'Pose', 'Classify', 'RTDETRDecoder', 'Ocr']
...
class Ocr(Detect):
    """YOLOv8 Ocr head for Ocr models."""

    def __init__(self, nc=80, nin=512, npr=256, seq_class=40, img_h=8, ch=()):
        """Initialize the YOLO model attributes such as the number of masks, prototypes, and the convolution layers."""
        super().__init__(nc, ch)
        self.npr = npr
        self.detect = Detect.forward
        ...
        self.ocr_proto = OcrProto(nin, npr)  # protos
        self.recognizer = CRNN(self.img_h, npr, seq_class, npr)  #
    ...
    def forward(self, features, batch=None):
        ...
        ocr_protos = self.ocr_proto(features[-1])
        detects_all = self.detect(self, x)
        transcripts = ()
        if self.training:
            # use ocr_proto and batch info to train crnn model
            # get text bboxes
            if batch is not None and batch.get("bboxes") is None:
                return detects_all, transcripts
            text_index = batch["ocrs"].sum(-1).gt_(0).bool()
            text_bboxes = batch["bboxes"][text_index]
            batch_idx = batch["batch_idx"][text_index]
            if text_index.sum() > 0:
                # convert text bboxes to ocr_protos size
                rois = ops.xywh2xyxy(text_bboxes.mul_(torch.tensor(ocr_protos.shape[2:])[[1, 0, 1, 0]]))
                # get pooled_width
                w = rois[:, 2] - rois[:, 0]
                h = rois[:, 3] - rois[:, 1]
                if h.any() <= 0 or w.any() <= 0:
                    return detects_all, transcripts
                ratios = w / h
                maxratio = ratios.max().item()
                pooled_width = np.ceil(self.img_h * maxratio).astype(int)
                if pooled_width >= self.max_width:
                    ratios = (self.max_width / pooled_width) * ratios
                    pooled_width = self.max_width
                # get text bboxes features
                boxes = torch.cat((batch_idx.unsqueeze(-1), rois), 1).to(ocr_protos.device)
                text_features = torchvision.ops.roi_align(ocr_protos, boxes, (self.img_h, pooled_width))
                text_lengths = torch.ceil(self.img_h * ratios).int()
                pred_ocrs = self.recognizer(text_features)
                transcripts = (pred_ocrs, text_lengths)
            return detects_all, transcripts
        elif self.export:
            y = detects_all
        else:   # inference or validation
            y, detects = detects_all    # y: batch_size, 4+num_classes, num_boxes   xywh
        preds = ops.non_max_suppression(y, classes=self.classes, conf_thres=self.conf_thres, iou_thres=self.iou_thres)  # (x1, y1, x2, y2, confidence, class, mask1, mask2, ...)
        pred_ocrs, pred_lens, text_boxes = [], [], []
        imgsz = torch.tensor(self.shape[2:], device=self.stride.device) * self.stride[0]
        for pred_box in preds:
            if len(pred_box) == 0:
                text_boxes.append(pred_box)
                pred_lens.append(torch.zeros(0,device=self.stride.device))
                pred_ocrs.append(torch.zeros(0, 0, self.seq_class, device=self.stride.device))
                continue
            rois = ops.scale_boxes((imgsz[0], imgsz[1]), pred_box[:, :4].clone(), ocr_protos.shape[2:])     # scale boxes ocr_protos shape
            # get pooled_width
            w = rois[:, 2] - rois[:, 0]
            h = rois[:, 3] - rois[:, 1]
            if h.le(0).any() or w.le(0).any():
                text_boxes.append(pred_box)
                pred_lens.append(torch.zeros(0,device=self.stride.device))
                pred_ocrs.append(torch.zeros(0, 0, self.seq_class, device=self.stride.device))
                continue
            ...
            if pooled_width >= self.max_width:
                ...
                pooled_width = self.max_width
            # get text bboxes features
            text_features = torchvision.ops.roi_align(ocr_protos, [rois], (self.img_h, pooled_width))
            text_length = torch.ceil(self.img_h * ratios).int()
            pred_ocr = self.recognizer(text_features)      # 对齐OCR boxes 是按照顺序对齐的
            pred_ocrs.append(pred_ocr)
            pred_lens.append(text_length)
            text_boxes.append(pred_box)

        transcripts = (pred_ocrs, pred_lens, text_boxes)
        if self.export:
            return y, transcripts
        else:
            return y, detects, transcripts